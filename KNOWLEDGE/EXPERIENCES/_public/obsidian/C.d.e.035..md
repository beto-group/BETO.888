...
const initialPlaceholders = {
  ...
  headerTitle: "Recipe Viewer", // Header title for the main view, controllable from the control panel
};

const initialGroupBySettings = {
  order: "asc", // Default order for group by, can be 'asc' or 'desc'
};
    ...
    >
      ...
      <div style={styles.editBlockButtonGroup}>
        ...
        <dc.Button
          onClick={() => {
            if (groupByColumn === columnId) {
              ...
              setGroupOrder(initialGroupBySettings.order); // Set the default group order
            }
          }}
          style={{
            ...styles.button,
        ...
        {groupByColumn === columnId && (
          <dc.Button
            onClick={() => setGroupOrder(groupOrder === "asc" ? "desc" : "asc")}
            style={{ ...styles.button, ...styles.buttonFullWidth }}
          >
            {groupOrder === "asc" ? "Order Desc" : "Order Asc"}
          </dc.Button>
        )}
      </div>
    </div>
...
function View() {
  ...
  const [groupOrder, setGroupOrder] = useState(initialGroupBySettings.order);
  ...
  if (groupByColumn) {
    ...
    // Convert groups object to array and sort by key based on order (asc or desc)
    displayedData = Object.keys(groups)
      .sort((a, b) => groupOrder === "asc" ? a.localeCompare(b) : b.localeCompare(a))
      .map((key) => ({
        key,
        items: groups[key],
      }));
