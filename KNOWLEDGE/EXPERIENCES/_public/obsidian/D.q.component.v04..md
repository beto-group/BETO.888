...
function EditableHeader({ columnId, editedHeaders, setEditedHeaders }) {
  ...
  const handleBlur = () => {
    ...
    setEditedHeaders((prev) => ({
      ...prev,
      [columnId]: headerValue.trim() || columnId,
    }));
  };
...
function PaginationSettings({
  ...
  return (
    <div style={styles.paginationSettingsContainer}>
      ...
      />
      {isEnabled && (
        <div style={styles.paginationSettingsControls}>
...
function View({ initialSettingsOverride = {} }) {
  ...
  const totalEntries = groupedData.filter((item) => item.type !== "group").length;
...
// Return the View component as part of an object
return { View };
```