...
function View({ initialSettingsOverride = {}, app }) {
  ...
  useEffect(() => {
    const helperSuggestions = [
      "#tags", "#source", "#genre", "#ingredients" // Show basic frontmatter fields as suggestions
    ];

    if (queryInput.startsWith("#")) {
      try {
        // Using Obsidian properties to retrieve list of frontmatter keys dynamically
        const allFrontmatterKeys = ["tags", "source", "genre", "ingredients"];
        const matchedKeys = allFrontmatterKeys.filter(key => key.startsWith(queryInput.slice(1).toLowerCase()));
        const tagSuggestions = matchedKeys.map(key => `#${key}`);
        helperSuggestions.push(...tagSuggestions);
      } catch (error) {
        console.error("Failed to retrieve frontmatter keys for suggestions:", error);
      }
    }
  ...
  const handleSuggestionClick = (suggestion) => {
    setQueryInput((prev) => `${prev} ${suggestion}`.trim());
    setShowSuggestions(false);
