 ...
 */
function MCPBridge({ folderPath, containerRef, onReload, onRunTests, debugManager, isTelegramEnabled }) {
    ...
    useEffect(() => {
        if (isTelegramEnabled) {
            if (apiRef.current.init) apiRef.current.init();
        } else {
            if (apiRef.current.disconnect) apiRef.current.disconnect();
        }
    }, [isTelegramEnabled]);
    ...
    useEffect(() => {
        ...
        const logActivity = (a, b, c) => {
            ...
            if (typeof c === 'undefined') {
                ...
                logToFile(folderPath + '/_resources/data/mcp_activity.json', { action: a, status: b, result: "" });
            } else {
                // Style: logActivity(source, action, result)
                logToFile(folderPath + '/_resources/data/mcp_activity.json', { source: a, action: b, result: maskedC, status: 'success' });
            }
        };

        const logConsole = (type, args) => logToFile(folderPath + '/_resources/data/mcp_logs.json', { type, message: args.map(a => String(a)).join(' ') });

        const addLogEntry = (source, level, message) => {
            logToFile(folderPath + '/_resources/data/mcp_logs.json', { source, level, message });
        };
        ...
        apiRef.current.init = initTelegram;
        apiRef.current.disconnect = async () => {
            if (tgBridgeRef.current) {
                try { await tgBridgeRef.current.disconnect(); } catch (e) {}
            ...
            try {
                ...
                if (cmdData && cmdData.executed === false) {
                    ...
                    if (cmdData.action === 'telegram_nuclear_reset') {
                        try {
                            const files = [
                                folderPath + '/_resources/data/telegram_session.txt',
                                folderPath + '/_resources/data/telegram_session_user.txt',
                                folderPath + '/_resources/data/telegram_session_bot.txt'
                            ];
