...
async function View({ folderPath }) {
    ...
    const Agent = {
        ...
        start: (fPath, onReload) => {
            ...
            const logFile = fPath + '/_resources/data/mcp_activity.json';
            ...
            return (
                <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#666' }}>
                    <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                        ...
                        Loading Universal Storage Showcase...
                    </div>
...
function AppWrapper({
    ...
    const [isTelegramEnabled, setIsTelegramEnabled] = dc.useState(false);

    dc.useEffect(() => {
        try {
            const val = localStorage.getItem(`dc_telegram_enabled_${folderPath}`);
            if (val === 'true') setIsTelegramEnabled(true);
        } catch (e) { }
    }, [folderPath]);
    ...
    const toggleTelegram = () => {
        ...
        try { localStorage.setItem(`dc_telegram_enabled_${folderPath}`, newVal); } catch (e) {}
    };
