/**
 * 128_Native_Grab - DEFINITIVE VERSION (v240325_0440)
 * Replaces react-grab with native CDP + Immersive FullTab.
 */
async function View({ folderPath }) {
    ...
    const STYLES = {
        ...
        drag: (drag, x, y) => ({ position: 'absolute', width: '120px', height: '120px', left: x, top: y, background: drag ? 'rgba(139, 92, 246, 0.4)' : 'rgba(139, 92, 246, 0.15)', backdropFilter: 'blur(10px)', border: '1px solid rgba(139, 92, 246, 0.6)', borderRadius: '16px', cursor: drag ? 'grabbing' : 'grab', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '8px', transition: drag ? 'none' : 'all 0.15s cubic-bezier(0.2, 0.8, 0.2, 1)', boxShadow: drag ? '0 30px 60px rgba(0,0,0,0.6), 0 0 30px rgba(139, 92, 246, 0.4)' : '0 10px 20px rgba(0,0,0,0.4)', zIndex: 200 }),
    ...
    const App = () => {
            ...
            const check = async () => {
                ...
                try {
                    ...
                    if (c && !c.executed) {
                        ...
                        if (c.action === 'screenshot') execute(`dev:screenshot path="${folderPath}/screenshots/mcp_${Date.now()}.png"`);
        ...
        useEffect(() => {
            ...
            const up = () => { if (drag) { setDrag(false); addLog("RELEASE"); } };
        ...
        return (
            <div ref={rootRef} id="native-grab-definitive" style={STYLES.main}>
                ...
                <main style={STYLES.canvas}>
                    ...
                    <div onMouseDown={(e) => { setDrag(true); setOff({ x: e.clientX - pos.x, y: e.clientY - pos.y }); addLog("LOCK_ON"); }} style={STYLES.drag(drag, pos.x, pos.y)}>
