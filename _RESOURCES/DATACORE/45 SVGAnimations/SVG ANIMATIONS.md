...
- **Can’t**:
    ...
    - **Auto-Generation Logic**: The component applies existing CSS to an SVG. It does not automatically create new animation keyframes or styles from a static SVG.
    - **Session Persistence**: Any SVG or CSS code entered into the "Build Your Own" tool is for the current session only and is not saved.
    - **Offline Video Export**: The video export feature requires an internet connection on its first use to download and cache the Canvg rendering library.
    - **Raster Animation Support**: The component is designed exclusively for vector-based SVG graphics and cannot animate formats like PNG or JPG.
