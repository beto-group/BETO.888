...
- **Component Breakdown**:
    ...
    - `src/utils/domUtils.jsx`: Standard DOM traversal helpers.
    - `src/styles/styles.jsx`: Design tokens (standardize export as `STYLES`).
- **View Factory**: The main `index.jsx` should act as a factory that loads dependencies, manages the Full-tab lifecycle (via portal breakout), and returns the root `ViewComponent`.

## 🖥️ Full-tab Integration
- **Edge-to-Edge UI**: Components should occupy the entire Obsidian tab, hiding default margins and padding.
- **Portal Breakout Pattern**: Implement `useEffect` in `index.jsx` to move the component to `.workspace-leaf-content`. This guarantees dimensions for WebGL renderers.
- **Relocation Lifecycle**: Standardize on `findNearestAncestorWithClass(container, "workspace-leaf-content")` to find the mount point.
- **Status Bar Toggle**: Hide `.app-container .status-bar` on mount; restore on unmount.

## 🔌 Viewer Files
- **Viewer Component Syntax**: Use `datacorejsx` codeblocks. Use `dc.resolvePath` with the component name for stable resolution.

## 🛠️ Library Export Handling
- **ESM Adapters**: When loading libraries like `OrbitControls` or `lil-gui` via CDNs, always check for both named and default exports:
  ```javascript
  const mod = await loadScript(dc, '...', { type: 'module' });
  const Component = mod.Component || mod.default?.Component || mod.default;
  ```
- **Dimension Guards**: Always check `container.clientWidth` in `init()`. If 0, wait 500ms for Obsidian to finish layout before initializing Three.js.
...
## 🪟 Advanced Full-Tab Immersion (D3/WebGL)
When absolute positioning is required (such as with D3 bindings) directly inside the container without being restricted by Obsidian parent elements:

1. **DOM Traversal**: Break out of standard DOM wrappers by walking up parents.
   ```javascript
    ...
    // Search for "workspace-leaf-content" to append the container explicitly
   ```
2. **Component Wrapper CSS**: The React `style` for the full-tab wrapper component should ALWAYS use `position: 'absolute'` and `inset: '0'` rather than just 100% height to guarantee edge-to-edge rendering without vertical/horizontal overflow when injected into `workspace-leaf-content`. 
    - ❌ `position: 'relative', height: '100%'` (will respect margin restrictions of markdown leaf)
    - ✅ `position: 'absolute', inset: '0', overflow: 'hidden'` (breaks out seamlessly)
