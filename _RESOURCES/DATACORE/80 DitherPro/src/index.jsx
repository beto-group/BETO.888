...
async function View({ folderPath, isInception, dc, ...props }) {
    ...
    const { DitherComponent } = await dc.require(folderPath + '/src/components/DitherComponent.jsx');

    // LoadScript utility for Tweakpane (as ESM module)
    ...
    return function ViewComponent() {
        ...
        const [isFullTab, setIsFullTab] = useState(!isInception); // Default to full tab unless in inception
        ...
        const toggleFullTab = () => {
            if (isInception) return; // Cannot full-tab in inception mode
            setIsFullTab(!isFullTab);
        };

        // Full-tab mode lifecycle
        useEffect(() => {
            // If closed (Compact Mode) or inception, remain in original flow (don't portal)
            ...
            return () => {
                console.log("Datacore: Cleaning up Full Tab Mode (DitherPro)");
        ...
        return (
            <div ref={containerRef} style={{ width: '100%', height: effectiveFullTab ? '100%' : '600px', backgroundColor: '#050505', borderRadius: effectiveFullTab ? '0' : '8px', overflow: 'hidden' }}>
                <DitherComponent
                    dc={dc}
