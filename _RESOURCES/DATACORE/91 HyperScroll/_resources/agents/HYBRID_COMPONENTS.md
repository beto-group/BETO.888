...
3.  **Strict State Management for WebGL**: 
    ...
    - Never place them in `useState`, as React's re-render cycle will desync from the browser's 60fps `requestAnimationFrame` loop.
    - Guarantee cleanup (disposing materials, geometries, renderers, and canceling animation frames) in the returning `useEffect` destructor. This prevents massive memory leaks during hot reloads or route changes.
