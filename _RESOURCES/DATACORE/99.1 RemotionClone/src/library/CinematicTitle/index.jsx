...
function CinematicTitle(props) {
    ...
    const entryOpacity = clamp(frame / 20, 0, 1);

    // 2. Tracking Animation (0-150f)
    // Slowly expands letters
    const trackingProgress = easeOutExpo(clamp(frame / 150, 0, 1));
    const letterSpacing = 2 + (trackingProgress * 12);

    // 3. Subtext Reveal (40-100f)
    const subtextProgress = easeOutExpo(clamp((frame - 40) / 60, 0, 1));
    const subtextOpacity = subtextProgress;
    const subtextY = 20 - (subtextProgress * 20);
    ...
    const STYLES = {
        ...
        title: {
            ...
            whiteSpace: 'nowrap',
            transition: 'none' // We control via frame
        },
        subtext: {
            fontSize: '18px',
            fontWeight: '400',
