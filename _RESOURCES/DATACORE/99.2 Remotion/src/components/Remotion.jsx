 ...
 * 
 * REFACTORED: Logic extracted to custom hooks and uses RemotionProvider.
 */
...
function Remotion({
    ...
    folderPath,
    RemotionProvider,
    bridge, // Single source of truth for official libs
    RootComposition,
    ...props
}) {
    const { useRef, useState, useEffect, useCallback, useMemo } = dc;
        ...
        return (
            <div style={STYLES.compactWrapper} className={uniqueWrapperClass}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={STYLES.subtitle}><strong>Remotion</strong> ({instanceId})</span>
                    <div
                ...
                >
                    ...
                    >
                        ...
                        <div ref={stageElementRef} style={{ width: '100%', height: '100%', position: 'relative', overflow: 'hidden', background: '#000000' }} id="remotion-stage">
                            ...
                            >
                                <PlayerBridge
                                    officialPlayer={bridge.officialPlayer}
                                    officialRemotion={bridge.officialRemotion}
                                    officialReact={bridge.officialReact}
                                    officialReactDOM={bridge.officialReactDOM}
...
/**
 * PlayerBridge
 * Manually mounts the React-based Remotion Player into the Preact-based Datacore view.
 * This isolates the renderers and prevents hook conflicts.
 */
function PlayerBridge({
    ...
    dc.useEffect(() => {
        if (!containerRef.current || !officialPlayer?.Player || !R || !RootComposition) return;

        if (!rootRef.current && RD.createRoot) {
            rootRef.current = RD.createRoot(containerRef.current);
        }

        const Player = officialPlayer.Player;

        const content = R.createElement(Player, {
            component: RootComposition,
            inputProps: {
                React: R, // Pass official React
                remotion: officialRemotion, // Pass official Remotion
                activeBackground,
                libraryComponents,
                sequence,
                frame,
            ...
            frame, // Sync with external frame if needed
            controls: false
        });
        ...
        if (rootRef.current) {
            ...
            RD.render(content, containerRef.current);
        }
    }, [officialPlayer, officialRemotion, RootComposition, frame, sequence, activeBackground, isPlaying, width, height]);

    return dc.preact.createElement('div', {
        ref: containerRef,
        style: { width: '100%', height: '100%' }
    });
}
